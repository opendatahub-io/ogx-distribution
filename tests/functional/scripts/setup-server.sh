...
for p in data:
    ...
    if api not in api_map:
        api_map[api] = pid
if 'inference' in api_map:
    print(f'INFERENCE_PROVIDER={api_map[\"inference\"]}')
if 'files' in api_map:
    print(f'FILES_PROVIDER={api_map[\"files\"]}')
if 'vector_io' in api_map:
    print(f'VECTOR_IO_PROVIDER={api_map[\"vector_io\"]}')
" <<< "$response")"
